...
- **Status:** Approved contract; Publish와 public renderer는 미구현
...
참고문헌은 별도 mutable source record가 아니라 해당 `content_versions.migration_metadata.knowledge.references`에 version payload로 저장한다. 따라서 새 Draft version이 만들어져도 이전 version의 참고문헌은 변경되지 않는다.

각 항목은 `type, title, authorOrOrganization, url, accessedAt, note`를 가진다. API/DB JSON의 camelCase 이름은 UI의 `author_or_organization`, `accessed_at` 의미와 같다.
...
- Publish는 Stage 17 이후 별도 명시적 owner Server Action으로만 요청한다.
...
새 Knowledge는 `/archive/{slug}` 또는 `route_scope='archive'`를 사용하지 않는다. 현재 `/archive/**`는 기존 Markdown renderer의 legacy URL이며 새 CMS와 암묵적으로 연결하지 않는다. Stage 16에서는 `/knowledge` public route를 구현하지 않는다.
...
- external 또는 direct_verification 참고문헌이 하나도 없음
- 참고문헌 구조·필수 필드·URL이 잘못됨
...
태그 없음은 Publish를 차단하지 않는다. Stage 16의 `validateKnowledgePublishReadiness`는 결과를 `{ blockingErrors, warnings, ready }`로 반환하지만 실제 Publish를 실행하지 않는다.
...
- `/knowledge`와 `/knowledge/{slug}`가 published pointer만 조회하는 renderer
