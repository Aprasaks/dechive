...
async function apply(url: string) {
  ...
  try {
    ...
    const names = (await readdir(path.resolve('drizzle'))).filter((name) => /^000[0-8].*\.sql$/.test(name)).sort();
    if (names.length !== 9) throw new Error('blocked_expected_migration_set_missing');
    ...
    const status = await pool.query<{ count: string; checksum: string }>(`SELECT count(*)::text AS count,
      ...
      FROM schema_migrations WHERE name ~ '^000[0-8]'`);
...
async function main() {
  ...
  try {
    ...
    if (first.migrationCount !== 9 || !report.secondRunNoOp) process.exitCode = 2;
  } catch (error) {
