...
async function apply(url: string) {
  ...
  try {
    ...
    const names = (await readdir(path.resolve('drizzle'))).filter((name) => /^000[0-3].*\.sql$/.test(name)).sort();
    if (names.length !== 4) throw new Error('blocked_expected_migration_set_missing');
    ...
    const status = await pool.query<{ count: string; checksum: string }>(`SELECT count(*)::text AS count,
      ...
      FROM schema_migrations WHERE name ~ '^000[0-3]'`);
...
async function main() {
  ...
  try {
    ...
    if (first.migrationCount !== 4 || !report.secondRunNoOp) process.exitCode = 2;
  } catch (error) {
