...
import { getSearchConsoleReport } from '@/lib/searchConsoleClient';
...
function Stat({ label, value, note }) {
  ...
  return <div className="border border-border bg-background p-4"><p className="text-xs font-semibold text-muted-foreground">{label}</p><p className="mt-3 text-3xl font-semibold text-foreground">{displayValue}</p>{note ? <p className="mt-2 text-xs leading-5 text-muted-foreground">{note}</p> : null}</div>;
}
...
function formatPosition(value) { return value ? value.toFixed(1) : '—'; }
function searchConsoleMessage(status) {
  if (status === 'not_configured') return 'Search Console 속성 URL 또는 Google 서비스 계정 인증이 설정되지 않았습니다.';
  if (status === 'permission_denied') return '서비스 계정에 Search Console 속성 읽기 권한을 추가해야 합니다.';
  if (status === 'data_delayed') return '오늘은 Search Console 확정 데이터가 아직 없어 어제 이전 기간을 선택해 주세요.';
  if (status === 'unavailable') return 'Search Console 데이터를 일시적으로 불러오지 못했습니다.';
  return '';
}

function Search({ report, searchConsole }) {
  const connected = searchConsole.status === 'connected';
  return <div className="space-y-6"><div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]"><Section title="사이트 검색" eyebrow="Search Intent"><div className="grid gap-3 sm:grid-cols-2"><Stat label="검색 제출" value={report.search.totalSearches} /><Stat label="결과 0건" value={report.search.zeroResultSearches} /></div><p className="mt-5 text-sm leading-6 text-muted-foreground">사이트 검색어는 운영 개선을 위해 집계합니다. Google·Naver의 개별 검색어와 외부 AI 프롬프트는 브라우저에서 직접 받을 수 없습니다.</p></Section><Section title="무엇을 찾지 못했나" eyebrow="Own Search Queries">{report.search.queries.length ? <div className="overflow-x-auto"><table className="w-full min-w-[500px] text-left text-sm"><thead className="border-b border-border text-xs text-muted-foreground"><tr><th className="pb-3">검색어</th><th className="pb-3">검색</th><th className="pb-3">0건</th><th className="pb-3">결과 클릭</th></tr></thead><tbody className="divide-y divide-border">{report.search.queries.map((item) => <tr key={item.query}><td className="max-w-[260px] truncate py-3">{item.query}</td><td className="py-3">{formatNumber(item.searches)}</td><td className="py-3">{formatNumber(item.zeroResults)}</td><td className="py-3">{formatNumber(item.resultClicks)}</td></tr>)}</tbody></table></div> : <EmptyState>검색 이벤트가 아직 없습니다.</EmptyState>}</Section></div><Section title="Google 검색 성과" eyebrow="Search Console"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">{connected ? `${searchConsole.startDate} ~ ${searchConsole.endDate}` : searchConsoleMessage(searchConsole.status)}</p>{searchConsole.siteUrl ? <a className="text-sm font-semibold text-accent underline" href={`https://search.google.com/search-console?resource_id=${encodeURIComponent(searchConsole.siteUrl)}`} target="_blank" rel="noopener noreferrer">Search Console 열기</a> : null}</div>{connected ? <><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Stat label="클릭" value={searchConsole.summary.clicks} /><Stat label="노출" value={searchConsole.summary.impressions} /><Stat label="CTR" value={formatPercent(searchConsole.summary.ctr)} /><Stat label="평균 순위" value={formatPosition(searchConsole.summary.position)} /></div><p className="mt-4 text-xs text-muted-foreground">{searchConsole.dataNote} Search Console API는 상위 행 중심으로 반환하므로 표의 합계가 전체 속성 합계와 다를 수 있습니다.</p><div className="mt-6 grid gap-6 lg:grid-cols-2"><div><h3 className="text-sm font-semibold text-foreground">상위 검색어</h3>{searchConsole.queries.length ? <div className="mt-3 divide-y divide-border">{searchConsole.queries.slice(0, 10).map((item) => <div key={item.key} className="flex items-center justify-between gap-4 py-2 text-sm"><span className="truncate">{item.key}</span><span className="shrink-0 text-muted-foreground">{formatNumber(item.clicks)} 클릭 · {formatNumber(item.impressions)} 노출</span></div>)}</div> : <EmptyState>검색어 데이터가 없습니다.</EmptyState>}</div><div><h3 className="text-sm font-semibold text-foreground">상위 페이지</h3>{searchConsole.pages.length ? <div className="mt-3 divide-y divide-border">{searchConsole.pages.slice(0, 10).map((item) => <div key={item.key} className="flex items-center justify-between gap-4 py-2 text-sm"><span className="truncate">{item.key}</span><span className="shrink-0 text-muted-foreground">{formatNumber(item.clicks)} 클릭 · {formatNumber(item.impressions)} 노출</span></div>)}</div> : <EmptyState>페이지 데이터가 없습니다.</EmptyState>}</div></div><div className="mt-6 border-t border-border pt-5"><h3 className="text-sm font-semibold text-foreground">노출은 있지만 클릭이 약한 검색어</h3>{searchConsole.opportunities.length ? <div className="mt-3 divide-y divide-border">{searchConsole.opportunities.map((item) => <div key={item.key} className="flex items-center justify-between gap-4 py-2 text-sm"><span className="truncate">{item.key}</span><span className="shrink-0 text-muted-foreground">{formatNumber(item.impressions)} 노출 · {formatPercent(item.ctr)} CTR · {formatPosition(item.position)}위</span></div>)}</div> : <p className="mt-3 text-sm text-muted-foreground">현재 조건에 맞는 개선 후보가 없습니다.</p>}</div></> : <p className="mt-5 border border-amber-300 bg-amber-50 px-4 py-4 text-sm leading-6 text-amber-900">{searchConsoleMessage(searchConsole.status)}</p>}</Section></div>;
}
