...
export async function generateStaticParams() {
  return getDeepDivePosts('en').map((post) => ({ slug: post.slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonical = `${BASE_URL}/en/deep-dive/${slug}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        ko: `${BASE_URL}/deep-dive/${slug}`,
        en: canonical,
        'x-default': `${BASE_URL}/deep-dive/${slug}`,
      },
    },
    openGraph: {
      title: post.title,
...
export default async function EnDeepDivePostPage({ params }: PageProps) {
  const { slug } = await params;
  const post = getDeepDivePost(slug);

  if (!post) notFound();

  const canonical = `${BASE_URL}/en/deep-dive/${post.slug}`;
  const image = resolvePostImage(post.coverImage || post.thumbnail);
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              { '@type': 'ListItem', position: 2, name: 'Deep Dive', item: `${BASE_URL}/en/deep-dive` },
