...
import { ChevronDown, Compass, Database, Sparkles } from 'lucide-react';
...
function groupPostsByYear(posts: Post[]) {
  return posts.reduce<Array<{ year: string; posts: Post[] }>>(
    (groups, post) => {
...
export default function BookArchive({
  ...
  return (
    <section
      className={`relative min-h-[calc(100vh-5rem)] flex-1 overflow-visible bg-[#f8f6f1] text-[#17120d] lg:h-[calc(100vh-5rem)] lg:overflow-hidden ${sansFontClassName}`}
    >
      <div
        aria-hidden="true"
      ...
      <div className="grid min-h-[calc(100vh-5rem)] lg:h-full lg:min-h-0 lg:grid-cols-[15.5rem_1fr] xl:grid-cols-[17rem_1fr]">
        <aside className="hidden h-full overflow-hidden border-r border-[#ded6c9] bg-[#f2eee6]/70 px-6 pt-8 pb-12 sm:px-7 lg:block lg:px-8">
          <div>
            <p
              className={`text-sm font-semibold tracking-[0.08em] text-[#8a6332] uppercase ${serifFontClassName}`}
            >
              Browse Archives
            </p>
            ...
            <div className="mt-10">
              <p
                className={`text-base font-semibold tracking-[0.03em] text-[#17120d] uppercase ${serifFontClassName}`}
              >
                Categories
              </p>
                  ...
                  return (
                    ...
                    >
                      <Icon
                        ...
                        strokeWidth={1.6}
                        className={
                          isSelected ? 'text-[#6f461d]' : 'text-[#8a6332]'
                        }
                      />
                      {label}
                    </button>
        ...
        <div className="px-6 py-8 sm:px-8 lg:h-full lg:overflow-y-auto lg:px-14 lg:py-16 xl:px-20">
          <div className="mx-auto max-w-6xl">
            <div className="mb-8 lg:hidden">
              ...
              <div
                className={`${isMobileCategoryOpen ? 'grid' : 'hidden'} mt-2 gap-1 border border-[#ded6c9] bg-[#f8f6f1] p-2 shadow-[0_18px_50px_rgba(42,33,27,0.08)]`}
              >
                {indexCategories.map(({ label, Icon }) => {
                  const isSelected = selectedCategory === label;
            ...
            <div className="border-b border-[#ded6c9] pb-12">
              <p
                className={`max-w-xl text-2xl leading-snug text-[#2b2119] ${serifFontClassName}`}
              >
                {lang === 'en'
                ...
                {groupedPosts.length > 0 ? (
                  groupedPosts.map((group) => (
                    <section key={group.year}>
                      <h3
                        className={`text-4xl font-medium tracking-[-0.03em] text-[#17120d] ${serifFontClassName}`}
                      >
                        {group.year}
                      </h3>
                      <div className="mt-6 divide-y divide-[#ded6c9] border-t border-[#ded6c9]">
                        {group.posts.map((post) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`flex flex-col items-center text-[#8a6332] ${serifFontClassName}`}
                              >
                              ...
                              <span>
                                <span
                                  className={`block text-2xl leading-snug text-[#17120d] ${serifFontClassName}`}
                                >
