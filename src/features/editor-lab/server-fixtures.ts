...
type MdNode = { type: string; value?: string; depth?: number; lang?: string; meta?: string; url?: string; alt?: string; title?: string; ordered?: boolean; start?: number; checked?: boolean; align?: Array<string | null>; children?: MdNode[] };
...
function convert(node: MdNode): JSONContent {
...
export async function loadEditorFixtures(): Promise<FixtureDocument[]> {
  ...
  for (const [id, label, sourcePath] of fixtureSpecs) {
    ...
    const converted = convert(parser.parse(parsed.content) as MdNode);
    converted.content = converted.content?.flatMap(flattenInvalidTextContainers);
    const document = normalizeAnchors({ ...converted, type: 'doc', schemaVersion: 1, content: converted.content ?? [] });
