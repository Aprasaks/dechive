...
- 라이트 테마만 제공한다.
- 따뜻한 종이색 배경과 짙은 먹색 본문을 기본으로 사용한다. 차분한 짙은 녹색은 링크와 분류에, 벽돌색은 날짜나 변화처럼 의미가 있을 때만 제한적으로 쓴다.
- 한글 본문과 인터페이스는 Pretendard Variable을 사용한다. 장시간 읽어도 피로하지 않도록 본문 크기와 줄간격을 충분히 확보한다.
